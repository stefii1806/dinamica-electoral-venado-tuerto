...
# CSS personalizado - ULTRA REFORZADO PARA TEMA CLARO
st.markdown("""
<style>
    ...
    * {
    ...
    /* Fondo blanco en TODA la aplicación */
    html, body, 
...
# Título principal
st.title("Dinámica Electoral en la Ciudad de Venado Tuerto (2019 - 2025) 🗳️")
...
with tab2:
    ...
    with col1:
        ...
        # Checkbox para agregar partidos adicionales
        ...
        # Partidos existentes en los datos base
        ...
        # Partidos adicionales si se activa la opción
    ...
    with col2:
        ...
        if abs(suma_actual - 100) < 0.5:
            ...
            for partido, pct in porcentajes_sim.items():
                if pct > 0:  # Solo incluir partidos con votos
                    votos_sim = int(votos_totales_sim * pct / 100)

                    # Buscar votos base (0 si es partido adicional)
            ...
            # Filtrar solo partidos que tienen base para comparar
            df_sim_con_base = df_sim[df_sim["Votos Base"] > 0]

            if not df_sim_con_base.empty:
                fig2 = go.Figure()

                colors_cambio = ["green" if x > 0 else "red" for x in df_sim_con_base["% Cambio"]]
