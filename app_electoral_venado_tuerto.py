...
with tab1:
    ...
    with subtab_local:
        ...
        # Checkbox DESPUÉS de seleccionar vista
        ...
        if vista_cuadrantes == "Total":
            ...
            fig.update_layout(
                ...
                yaxis_title="Porcentaje de Votos (%)",  # CAMBIO
                ...
                bargap=0.3,  # Más espacio entre grupos
                bargroupgap=0.1  # Menos espacio dentro de grupos
            )

            st.plotly_chart(fig, use_container_width=True)

        else:
            # Multiselect para cuadrantes
            ...
            if cuadrantes_sel:
                ...
                # Calcular total de votos por año/tipo/cuadrante para sacar porcentajes
                cuad_map_inv = {v: k for k, v in CUADRANTES.items()}

                fig = go.Figure()

                for cuadrante_nombre in cuadrantes_sel:
                    ...
                    for partido in df_cargo["Partido"].unique():
                        ...
                        fig.add_trace(go.Bar(
                            name=f"{partido} ({cuadrante_nombre})",
                            x=x_labels,
                            y=y_values,
                            marker_color=get_color(partido),
                            text=[f"{v:.1f}%" for v in y_values],
                            textposition="outside",
                            legendgroup=partido,
                            offsetgroup=cuadrante_nombre,
                            width=0.5  # Barras más angostas
                        ))

                fig.update_layout(
                    barmode="group",
                    title=f"Comparación: {', '.join(cuadrantes_sel)}",
                    height=500,
                    xaxis_title="Año y Tipo de Elección",
                    yaxis_title="Porcentaje de Votos (%)",  # CAMBIO
                    legend_title="Fuerza Política y Cuadrante",
                    hovermode="x unified",
                    plot_bgcolor="#ffffff",
                    paper_bgcolor="#f8f9fa",
                    font=dict(family="Inter, sans-serif", size=12, color="#2c3e50"),
    ...
    with subtab_provincial:
        ...
        if vista_cuadrantes_p == "Total":
            ...
            if cuadrantes_sel_p:
                ...
                for cuadrante_nombre in cuadrantes_sel_p:
                    cuadrante_codigo = cuad_map_inv[cuadrante_nombre]
                ...
                fig.update_layout(
                    barmode="group",
                    title=f"Comparación: {', '.join(cuadrantes_sel_p)}",
                    height=500,
                    xaxis_title="Año y Tipo de Elección",
                    yaxis_title="Porcentaje de Votos (%)",
                    legend_title="Fuerza Política y Cuadrante",
