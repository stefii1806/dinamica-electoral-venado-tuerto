...
@st.cache_data
def cargar_datos():
    df_participacion = pd.read_csv('Resultados-elecciones-TOTALES-participacion_hist.csv')
    df_participacion_cuad = pd.read_csv('Resultados-elecciones-TOTALES-participacion_hist_cuad.csv')
    df_resultados = pd.read_csv('Resultados-elecciones-TOTALES-agrup_def-1.csv')
...
with tab1:
    ...
    with subtab_local:
        ...
        st.subheader(f"Evolución de votos - {cargo_sel}")
        ...
        if vista_cuadrantes == "Total":
            ...
            fig.update_xaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1')
            fig.update_yaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1')
        ...
        st.subheader(f"Evolución de la participación electoral - {cargo_sel}")
        ...
        fig.update_xaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1')
        fig.update_yaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1', range=[0, 100])
    ...
    with subtab_provincial:
        ...
        st.subheader(f"Evolución de votos - {cargo_sel_p}")
        ...
        # Comparación específica: Gobernador 2023 vs Diputados N 2025 y Convencionales DÚ 2025
        st.markdown("---")
        ...
        with col_comp1:
            st.markdown("#### Gobernador 2023 (Generales) vs Diputados N 2025 (Generales)")
            ...
            for partido in partidos_comunes:
                ...
                if len(votos_2023) > 0 and len(votos_2025) > 0:
                    fig.add_trace(go.Bar(
                        name=partido,
                        x=['Gobernador 2023', 'Diputados N 2025'],
        ...
        with col_comp2:
            st.markdown("#### Gobernador 2023 (Generales) vs Convencionales DÚ 2025 (Primarias)")
            ...
            for partido in partidos_comunes2:
                ...
                if len(votos_2023) > 0 and len(votos_2025) > 0:
                    fig.add_trace(go.Bar(
                        name=partido,
                        x=['Gobernador 2023', 'Convencionales DÚ 2025'],
                        y=[votos_2023[0], votos_2025[0]],
                        marker_color=get_color(partido),
                        text=[votos_2023[0], votos_2025[0]],
                        textposition='outside',
...
with tab2:
    ...
    with col2:
        ...
        if abs(suma_actual - 100) < 0.5:
            ...
            fig2.update_xaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1')
            fig2.update_yaxis(showgrid=True, gridwidth=1, gridcolor='#ecf0f1')
